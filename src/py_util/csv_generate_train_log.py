...
dataset = "semeval" # "ustancebr" or "govbr"
...
for log_file_path in tqdm(glob(f"{log_base_path}/**/*.txt", recursive=True)):
    ...
    if data_folder in ["in_domain_filter"]:
...
df_results = df_results.dropna().query("data_folder in ['in_domain', 'hold1topic_out', 'in_domain_15k', 'in_domain_filter', 'hold1topic_out_filter']")
